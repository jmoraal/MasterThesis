...
class Disloc: 
    def __init__(self, index, loc, charge):
        self.idx = index #not sure if necessary
        self.x = loc
        self.b = charge
    
    def create(self, creationLoc, charge): 
        self.x = creationLoc
        self.b = charge
        self.t = 0 #time since creation. Do we need (absolute) creation time?
...
class Source: 
    ...
    def updateForce(self, PKforce, dt):
        self.PK = PKforce
        
        if np.abs(self.PK) > fnuc:
            self.tAboveThreshold += dt
            
            if self.tAboveThreshold > tnuc: #TODO is this the right place?
                Disloc.create(self.x - 0.5*Lnuc, np.sign(self.PK)) #TODO correct sign
                Disloc.create(self.x + 0.5*Lnuc, -np.sign(self.PK))
                self.tAboveThreshold = 0
        else: 
            self.tAboveThreshold = 0
    
    def incrThresTime(self, dt):
        self.tAboveThreshold += dt
